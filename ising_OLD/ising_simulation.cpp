...
int main(int argc, char* argv[]) {
    ...
    {
        printf("Usage: %s size(int) min_temperature(float) ", argv[0]);
        printf("change_temperature(float) num_lattices(int) ");
        printf("trials(int) mode(char)\n\n");
        exit(1);
    }

    for (int i = 0; i < n; i++) {
        Lattice *nextL = new Lattice(size, size, t + i * dt, mode);
